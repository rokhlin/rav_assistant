...
async def callback_translate(query: CallbackQuery, state: FSMContext):
    ...
    status_msg = await query.message.reply("⏳ *Выполняю перевод на русский язык...*", parse_mode="Markdown")

    try:
        result = ""
        if content_type == "image" and data.get("last_file_bytes"):
            image_bytes = bytes.fromhex(data["last_file_bytes"])
            mime = data.get("last_mime_type", "image/jpeg")
            result = await ai_service.translate_image(image_bytes, mime)
        elif data.get("last_extracted_text"):
            result = await ai_service.translate_text(data["last_extracted_text"])
        else:
            await status_msg.edit_text("⚠️ Исходные данные для перевода устарели. Пожалуйста, отправьте файл заново.")
            return

        kb = get_media_actions_keyboard(file_type=content_type or "doc", current_action="translate")
        await status_msg.edit_text(result, reply_markup=kb, parse_mode="Markdown")

    except Exception as e:
        logger.error(f"Ошибка повторного перевода: {e}")
        await status_msg.edit_text(f"❌ Ошибка перевода: {e}")
...
async def callback_analyze(query: CallbackQuery, state: FSMContext):
    ...
    status_msg = await query.message.reply("⏳ *Анализирую документ...*", parse_mode="Markdown")
    ...
    try:
        ...
        if content_type == "image" and data.get("last_file_bytes"):
            ...
            await status_msg.edit_text("⚠️ Исходные данные устарели. Пожалуйста, отправьте файл заново.")
            return
